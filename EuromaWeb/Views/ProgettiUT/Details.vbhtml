@ModelType EuromaWeb.ProgettiUTViewModel
<div class="row text-center">
    <div class="col">
        ...
        @If User.IsInRole("TecnicoAdmin") Or User.IsInRole("Admin") Then
            @<button type="button" data-type="edit_tecnico_admin" data-value="@Model.Id" Class="btn btn-primary w-auto" data-bs-toggle="modal" data-bs-target="#exampleModal">
                Assegnazione
        ...
        @If User.IsInRole("Tecnico") Or User.IsInRole("TecnicoRevisione") Or User.IsInRole("TecnicoAdmin") Or User.IsInRole("Admin") Then
            @<button type="button" data-type="edit_tecnico_operatore" data-value="@Model.Id" Class="btn btn-success w-auto" data-bs-toggle="modal" data-bs-target="#exampleModal">
                Stato Avanzamento
                <i class="fa-solid fa-file-pen"></i>
            </button>
            ...
            @If Model.StatoProgetto = Stato_Prod.Completato And Model.OC_Riferimento.Contains("OT") Then
                @<button onclick="SendToCommPrompt(@Model.Id);" type="button" data-value="@Model.Id" Class="btn btn-primary w-auto">
                    Invia a Uff. Comm.
                    <i class="fa-solid fa-paper-plane"></i>
                </button>
            ElseIf Model.StatoProgetto = Stato_Prod.Completato Then
                @<button onclick="SendToProdPrompt(@Model.Id);" type="button" data-value="@Model.Id" Class="btn btn-primary w-auto">
                    Invia a prod.
                    <i class="fa-solid fa-paper-plane"></i>
                </button>
            Else
                @<button onclick="SendDirectToProdPrompt(@Model.Id);" type="button" data-value="@Model.Id" Class="btn btn-primary w-auto">
                    Invia diretto
                    <i class="fa-solid fa-paper-plane"></i>
                </button>
                @<button Class="btn btn-primary w-auto disabled " data-toggle="tooltip" data-placement="top" title="Necessario stato Completato!">
                    Invia
                    <i class="fa-solid fa-paper-plane"></i>
                </button>
                    ...
                    <dl Class="dl-horizontal">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                @Html.EditorFor(Function(model) model.OperatoreSmistamento, New With {.htmlAttributes = New With {.class = "form-control", .readonly = "readonly"}})

                            </div>
                            <div class="col-md-6">
                                @Html.DisplayNameFor(Function(model) model.Operatore)
                                @Html.EditorFor(Function(model) model.Operatore, New With {.htmlAttributes = New With {.class = "form-control", .readonly = "readonly"}})
